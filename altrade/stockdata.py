...
class daysDataOfStock:
    def __init__(self, fullname, latestNbars=60):
        ...
        # self.DF.vol = self.DF.vol/10000.0  # for market index
        self.DF.vol = self.DF.vol/100.0  # for stocks
        self.DF.index.name = "date_idx"
        self.DF.loc[:, ['open', 'high', 'low', 'close']] = \
            self.DF.loc[:, ['open', 'high', 'low', 'close']]/100.0
    ...
    def getWeekDatum(self):
        ...
        return self.DF.resample('W-Fri', how=ohlc_dict, label='right')
         # DataFrame.resample('W-Fri', how=ohlc_dict)

class minuteOfStock:
    def __init__(self, fullname, latestNbars=480):
        minute5 = np.dtype({
        ...
        self.dataframe = pd.DataFrame(_datum1m)
        _DF = self.convertDateMin()
        self.DF = _DF[-1*latestNbars:]

    def convertDateMin(self):
        datenp = self.dataframe["monthdate"]
        minnp = self.dataframe["hourmin"]
        ...
        self.dataframe["datetime"] = pd.to_datetime(pd.Series(datetime_ary))
        ...
        self.dataframe.volume = self.dataframe.volume/100
        self.dataframe[['open', 'high', 'low', 'close']] =\
        np.round(self.dataframe[['open', 'high', 'low', 'close']], 2)
        return self.dataframe[['open', 'high', 'low', 'close', 'volume']]
    ...
    def resampleNminsDatum(self,intervals='15min'):
        ...
        return self.DF.resample(intervals, how=ohlc_dict, closed='right',label='right')
        # return self.DF.resample(intervals, how=ohlc_dict,loffset='-1m', label='right')
